...
This requires an efficient implementation in order that the merchant can make fast decisions.
We use the numpy library with its multidimensional arrays and vectorized array operations.


The dynamic programming approach is ....
%todo write about using multdimensional arrays for this
This was implemented using the numpy library.
Numpy is a library focused on numeric computation.
We use numpy's multidimensional arrays and vectorized array operations to implement the dynamic programming approach efficiently.

%how is demand learning implemented?
%todo more detail
We use the reliable linear regression implementation from the scikit-learn library.
The demand learning is implemented in a way that make it easy to add or change explanatory variables.
Only one function must be changed to add new explanatory variables.
