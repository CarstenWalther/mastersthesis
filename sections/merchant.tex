...
A changing customer behavior over time is out of scope of this work, thus the sale probability $P(i)$ does not depend on the time period $t$.
The sale probability in the first phase does not directly depend on the price or market situation because the merchant sells at a fixed price in a monopoly.
...
\todo{show solution for specific parameters, compare with/without delay}
...
\cref{fig:convergence} shows that dynamic programming converges faster if the start value $V_T$ is near the final expected profit.
