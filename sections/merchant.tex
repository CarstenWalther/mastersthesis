...
\subsection{Solution Approach}
%Idee: Effiziente dynamische Programmierung und erweiterte datenbasierte Nachfrageschätzung

%build on top of solution from previous section
%features ausbauen, da situatino complizierter geworden ist.
%describe features

%state is inventory level
%state could be market situation -> dimensions huge
% -> calculate new policy for new situation
...
Additionally, demand learning must predict transitions of competitors' offers from one time period to another.
This requires a substantial amount of trainings data to make sensible predictions.
%instead
%easier to train -> less data needed -> better predictions 


\subsection{Faster dynamic programming}
\label{section:faster_dyn_prog}
%Zeige, dass zu langsam. Zeige, wie schneller gemacht wird
%performance non-issue before, but now must be computed each iteration
%adapt search space
%use last expected profit
%tradeoff speed performance (genauigkeit))
%todo fix title
%not done here, quantisation
\subsection{Evaluation}
